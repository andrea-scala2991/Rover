...
wheel_speeds_t calculate_wheel_speeds(int PAD2_x_value, int PAD1_y_value,uint16_t sonar1,uint16_t sonar2,uint16_t sonar3) {
    ...
    if(PAD1_y_value >= PAD1_y_value_max_giu && PAD1_y_value <= PAD1_y_value_min_su){
    	...
    	if (PAD2_x_value >= 350 && PAD2_x_value <= 511) { //fermo gira verso sinitra
				...
				speeds.left_speed=-10;
				speeds.right_speed=10;
    ...
    /*char u[50];
    snprintf(u, sizeof(u), "lettura\r\n");
    HAL_UART_Transmit(&huart2, (uint8_t *)u, strlen(u), HAL_MAX_DELAY);*/
    return speeds;
}
...
//Funzione che fa anche la sterzata mentre si guida e non solo quando si ssta fermi.
/*wheel_speeds_t calculate_wheel_speeds(int setpoint, int x_value, int y_value) {
    wheel_speeds_t speeds;

    int steer_factor = 0;
    int set_point_steer= 0;

    // Calcolo il fattore sterzata basato sulla posizione dell'asse x
    if (x_value >= 281 && x_value <= 511) { // Sterzata verso sinistra in movimetno
    	speeds.right_speed=setpoint;
    	set_point_steer=(((x_value-281)*setpoint)/(511-281))*2;
    	speeds.left_speed=setpoint - set_point_steer;

    }
    else if (x_value >= 273 && x_value <= 280) { //fermo
    	speeds.left_speed=setpoint;
    	speeds.right_speed=setpoint;
    }
    else if(x_value >= 0 && x_value <= 273){ //sterzata verso destra in movimento
    	speeds.left_speed=setpoint;
    	set_point_steer=(((273-x_value)*setpoint)/(273-0))*2;
    	speeds.right_speed=setpoint - set_point_steer;

    }

    //gira su se stesso da fermo
    if(y_value >= 262 && y_value <= 268){
    	if (x_value >= 350 && x_value <= 511) { //fermo gira verso sinitra
        		speeds.left_speed=-10;
        	    speeds.right_speed=10;
            }
        	else if(x_value >= 0 && x_value <= 150){ //fermo gira verso destra
        	    speeds.left_speed=10;
        	   	speeds.right_speed=-10;
        	}
    }
    else if(y_value >= 0 && y_value <= 261){//se tiro in dietro la leveta si gira su se stesso di 180 gradi
    	speeds.left_speed=10;
    	speeds.right_speed=-10;
    }
    else if(y_value >= 269 && y_value <= 511){
    	speeds.left_speed=setpoint;
    	speeds.right_speed=setpoint;
    }
    ...
    // Calcolo delle velocità di ciascuna ruota in base alla sterzata
...
//LA CONSERVO SE SERVE MA INUTILER
